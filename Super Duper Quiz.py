...
def load_data():
    ...
    file_path = r"C:\Users\piyush.sharma4\Desktop\test\Super Duper Quiz\MCQ for test.csv"
    ...
    try:
        ...
        with open(file_path, 'r', encoding='latin1') as f:
            for line in f:
                ...
                # The CSV wraps every row in outer double-quotes — strip them.
                if line.startswith('"') and line.endswith('"'):
                    line = line[1:-1]

                # Use csv.reader to correctly split on commas
                parsed = next(csv_module.reader([line]))

                # Some questions contain a comma (e.g. "If mass doubles, force becomes")
                # which causes the row to parse as 11 columns instead of 10.
                # Fix: merge the question fragment (col 3) and the spill (col 4) back together.
                if len(parsed) == 11:
                    parsed = parsed[:3] + [parsed[3] + parsed[4]] + parsed[5:]
...
def isolated_timer_component():
    if st.session_state.step == "quiz" and st.session_state.quiz_state.get('end_time'):
        ...
        css_class = "timer-card-urgent" if remaining <= 30 else "timer-card"
    ...
    with col2:
        ...
        <div style="background-color: #1b2641; padding: 25px; border-radius: 15px; border: 1px solid #FFD700;">
            ...
            <ul style="color:#FFFFFF;">
                <li><b>Timer:</b> 20 seconds per question (pooled across the chapter).</li>
                <li><b>Explanations:</b> Shown immediately after you answer each question.</li>
        ...
        for i, ch in enumerate(chapters):
            if cols[i % 3].button(f"📘 {ch}", key=f"ch_{i}"):
                ...
                st.session_state.quiz_state = {
                    'idx': 0,
                    'answers': {},
                    'end_time': time.time() + (len(chapter_qs) * 20)
                }
...
# ── QUIZ ───────────────────────────────────────────────────────────────────────
    ...
    if idx not in qs['answers']:
        ...
        explanation = q_data.get('Explanation of Correct Answer', q_data.get('EXPLANATION of Correct Answer', 'N/A'))
        st.markdown(f"""
            <div class="explanation-box">
                <b style="color:#FFD700;">Explanation:</b><br>
...
# --- RESULTS ---
    ...
    st.markdown("---")
    st.markdown("<h3 style='color:#FFD700;'>📋 Answer Breakdown</h3>", unsafe_allow_html=True)
    for i, q in enumerate(chapter_qs):
        ans_data = qs['answers'].get(i)
        if ans_data:
            icon = "✅" if ans_data['correct'] else "❌"
            chosen_text = ans_data.get('chosen', '—')
        else:
            icon = "⏩"
            chosen_text = "Not attempted"
        with st.expander(f"{icon} Q{i + 1}: {q['Question'][:80]}..."):
            st.write(f"**Your answer:** {chosen_text}")
            st.write(f"**Correct answer:** {q['Correct Answer']}")
            exp = q.get('Explanation of Correct Answer', q.get('EXPLANATION of Correct Answer', 'N/A'))
